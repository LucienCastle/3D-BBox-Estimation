...
def compute_center(points3D,rot_M,cam_to_img,box_2D,inds):
    ...
    W = np.array([[fx, 0, u0 - box_2D[0]],
                  [fx, 0, u0 - box_2D[2]],
                  [0, fy, v0 - box_2D[1]],
                  [0, fy, v0 - box_2D[3]]], dtype=np.float)
    center =None
    error_min = 1e10

    for ind in inds:
        y = np.zeros((4, 1), dtype=np.float)
...
def draw_2D_box(image,points):
    points = points.astype(np.int)
    cv2.rectangle(image,tuple(points[0:2]),tuple(points[2:4]),(0, 0, 255), 2)
...
def gen_3D_box(yaw,dims,cam_to_img,box_2D):
    ...
    center, error = compute_center(points3D, rot_M, cam_to_img, box_2D, inds)

    points2D = points3D_to_2D(points3D, center, rot_M, cam_to_img)

    return points2D, error
